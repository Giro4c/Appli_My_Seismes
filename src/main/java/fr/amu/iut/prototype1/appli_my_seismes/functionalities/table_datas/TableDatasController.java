 ...
 */
public class TableDatasController {
    @FXML
    ...
    public void initialize() {
        ...
        MainController.getListeSeismesTries().addListener(listeTriesChangeListener);
    ...
    private void reloadData(){
        ...
        for (int indexSeisme = startIndex; indexSeisme < Math.min(startIndex + COUNT_LINES, MainController.getListeSeismesTries().size()); ++indexSeisme){
            listSeismesPage.add(MainController.getListeSeismesTries().get(indexSeisme));
        }
    ...
    private void setUpShowColumn(){
        // Reset showColumn pour éviter IndexOutOfBoundExeption (les bind resteront car attribués à numeros d'element dans la liste et non les élément eux même)
        showColumn.clear();
        // ID
    ...
    public void openParamsAffichage() throws IOException {
        FXMLLoader fxmlloaderFiltreAffich = new FXMLLoader(Main.class.getResource("functionalities/table_datas/DisplayFiltering.fxml"));
    ...
    public void openParamsTri() throws IOException {
        FXMLLoader fxmlLoader = new FXMLLoader(Main.class.getResource("functionalities/table_datas/Sorter.fxml"));
    ...
    private void refreshPagesNumber(){
        numPageActuelle.setValue(1);
        if (MainController.getListeSeismesTries() != null) {
            totalPages.setValue(MainController.getListeSeismesTries().size() / COUNT_LINES + 1);
        }
        else{
